...
var urlparse = function urlparse(urlStr) {
  var urlObj = url.parse(urlStr, true);
  urlObj.query = urlObj.query || {};
  return urlObj;
};

var isFirefox = function isFirefox(req) {
  if (!(req && req.headers)) {
    return false;
  } // Browser check
...
var getXUACompatibleMetaElement = function getXUACompatibleMetaElement(url) {
  var tag = '';
...
exports.includeScriptsIntoContext = function (files, log, injector, context, request) {
  ...
  var proxyPath = upstreamProxy ? upstreamProxy.path : '/'; // log.error('files', files);
  ...
  for (var i = 0; i < files.included.length; i++) {
    ...
    var scriptType = SCRIPT_TYPE[fileExt] || 'text/javascript'; // In case there is a JavaScript version specified and this is a Firefox browser
  ...
  var mappings = files.served.map(function (file) {
    ...
    var filePath = filePathToUrlPath(file.path, basePath, urlRoot, proxyPath).replace(/\\/g, '\\\\'); // Escape single quotes that might be in the filename -
...
exports.includeErrorIntoContext = function (errorFiles, log, injector, context) {
  ...
  return context.replace('%SCRIPTS%', '').replace('%CLIENT_CONFIG%', '').replace('%MAPPINGS%', 'window.__karma__.error("TEST RUN WAS CANCELLED because ' + (errorFiles.length > 1 ? 'these files contain' : 'this file contains') + ' some errors:\\n	 ' + errorFiles.join('\\n	 ') + '");');
};
